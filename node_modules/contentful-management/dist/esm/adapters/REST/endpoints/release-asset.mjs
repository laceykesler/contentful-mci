...
import { getUploadHttpClient } from '../../../upload-http-client.mjs';
import { post, put, get as get$1 } from './raw.mjs';
import { create as create$1 } from './upload.mjs';
import { normalizeSelect } from './utils.mjs';

const get = (http, params, rawData, headers) => {
    return get$1(http, `/spaces/${params.spaceId}/environments/${params.environmentId}/releases/${params.releaseId}/assets/${params.assetId}`, {
        params: normalizeSelect(params.query),
        headers: headers ? { ...headers } : undefined,
    });
};
const getMany = (http, params, rawData, headers) => {
    return get$1(http, `/spaces/${params.spaceId}/environments/${params.environmentId}/releases/${params.releaseId}/assets`, {
        params: normalizeSelect(params.query),
        headers: headers ? { ...headers } : undefined,
    });
};
const update = (http, params, rawData, headers) => {
    ...
    delete data.sys;
    return put(http, `/spaces/${params.spaceId}/environments/${params.environmentId}/releases/${params.releaseId}/assets/${params.assetId}`, data, {
        headers: {
...
const create = (http, params, rawData, headers) => {
    const data = copy(rawData);
    return post(http, `/spaces/${params.spaceId}/environments/${params.environmentId}/releases/${params.releaseId}/assets`, data, {
        headers,
    });
};
...
const createFromFiles = async (http, params, data) => {
    ...
    return Promise.all(Object.keys(file).map(async (locale) => {
        ...
        return create$1(httpUpload, params, file[locale]).then((upload) => {
            return {
                [locale]: {
                    contentType,
                    fileName,
...
const processForLocale = async (http, { asset, locale, options: { processingCheckRetries, processingCheckWait } = {}, ...params }) => {
    return put(http, `/spaces/${params.spaceId}/environments/${params.environmentId}/releases/${asset.sys.release.sys.id}/assets/${asset.sys.id}/files/${locale}/process`, null, {
        headers: {
...
export { create, createFromFiles, createWithId, get, getMany, processForAllLocales, processForLocale, update };
//# sourceMappingURL=release-asset.mjs.map
